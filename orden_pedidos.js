...
async function actualizarEstadoActual(){
  ...
  if(estado==='PREPARACION'){ voz('Pedido enviado a preparación'); }
  await cargarTodo();
  abrirModal(p.key);
}
  ...
  try{
    ...
    pedidos.forEach(p=>{
      ...
      if(old&&old!==p.alerta_token){
        hubo=true;
        toast('🔔 '+(p.alerta_mensaje||'Pedido enviado a preparación')+': '+p.pedido);
        voz('Pedido enviado a preparación');
      }
...
function pdfPedidoA4(){
  ...
  d.save('pedido_'+safe(p.pedido)+'_A4.pdf');
}
...
function pdfPedido80(){
  ...
  d.save('pedido_'+safe(p.pedido)+'_80mm.pdf');
}
