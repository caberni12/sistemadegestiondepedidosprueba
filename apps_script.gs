...
function doPost(e){
  const b = body_(e);
  const accion = t_(b.accion || b.action || '').toLowerCase();
  try{
    let r;
...
function actualizarEstadoPedido_(b){
  ...
  registrarMovimientoEstado_(pedido, estado, cliente, vendedor, pikeador, count, ts);

  if(estado === 'PREPARACION'){
    // Al pasar a preparación también dispara alerta global para todos los dispositivos.
    return alertaPedido_(Object.assign({}, b, {pedido:pedido, pikeador:pikeador}), 'PREPARACION', 'Pedido enviado a preparación', false);
  }

  return {ok:true,msg:'Estado actualizado correctamente',pedido:pedido,status:estado,pikeador:pikeador,actualizados:count,serverTime:new Date().toISOString()};
}
...
function alertaPedido_(b, tipo, mensaje, cambiarEstado){
  ...
  return {ok:true,msg:mensaje,pedido:pedido,actualizados:count,alerta_token:token,alerta_ts:ts,alerta_tipo:tipo,alerta_mensaje:mensaje,voice:'Pedido enviado a preparación'};
}
...
function registrarAlerta_(pedido,cliente,pikeador,tipo,mensaje,token,ts){
  const sh = getOrCreate_(SHEET_ALERTAS, ['id','fecha','pedido','cliente','pikeador','tipo','mensaje','token']);
  ensureCols_(sh, ['id','fecha','pedido','cliente','pikeador','tipo','mensaje','token']);
  sh.appendRow([id_('AL'),ts,pedido,cliente,pikeador,tipo,mensaje,token]);
}
